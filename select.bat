...
for %%t in (%threshold_score_list%) do (
	for %%c in (%chapters1%) do python SelectSentences.py %1 %subject1% %%c %a1% %b1% %thres_cossim1% %%t
)

for %%t in (%threshold_score_list%) do (
	for %%c in (%chapters2%) do python SelectSentences.py %1 %subject2% %%c %a2% %b2% %thres_cossim2% %%t
)
