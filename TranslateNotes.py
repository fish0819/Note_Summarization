...
SUBJECT = 'OM'
CHAPTER = 'supplementA'
# NOTE_FOLDER = 'note/OM/supplementA/'
NOTE_NAME = '生管_w3_黃慈方'
...
print (SUBJECT, CHAPTER, NOTE_FOLDER, NOTE_NAME)
CHNOTE_FILE_NAME = NOTE_NAME + '.docx'
ENNOTE_FILE_NAME = NOTE_NAME + '_en.txt'
CHEN_FILE_NAME = NOTE_NAME + '.csv'
...
with open(NOTE_FOLDER + CHEN_FILE_NAME, 'w', newline = '', encoding = 'utf-8') as csvFile:
    writer = csv.writer(csvFile, delimiter = ',')
    writer.writerow(['prev', 'post'])
    WordsList = []
    for term in Topics:
    ...
    for lid in range(len(note)):
        ...
        line = note[lid] = re.sub(' +', ' ', line)
        # outFile.write(line + '\n')
        writer.writerow([rawLine, line])