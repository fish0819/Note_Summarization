...
MIXED_FILE_NAME = 'Mixed_' + CHAPTER + '.csv'
...
# get note sentences
NoteFileNames = [(NOTE_FOLDER + f) for f in listdir(NOTE_FOLDER) if '.csv' in f and 'Mixed' not in f]
print (NoteFileNames)
EnNotes = []
ChNotes = []
Notes = []
for FILE_NAME in NoteFileNames:
    with open(FILE_NAME, 'r', encoding = 'utf-8') as inFile:
        ...
        Note = []
        for row in reader:
            Note.append({'ch_content': row['prev'].strip().replace('\n', ''), 'en_content': row['post'].strip().replace('\n', ''), 'topic': []})
            for term in Topics:
                if term['term_e'] in Note[-1]['en_content'] or (term['abbr'] != '' and (' ' + term['abbr'] + ' ' in Note[-1]['en_content'] or '(' + term['abbr'] + ')' in Note[-1]['en_content'])):
                    Note[-1]['topic'].append(term['term_e'])
    Notes.append(Note)
...
for nid in range(len(Notes)):
    for para in CombinedNotes[nid]:
        if len(MixedNoteParaList) == 0 or not any(np['sid'] == para['sid'] for np in MixedNoteParaList):
            MixedNoteParaList.append({'ch_content': '', 'en_content': '', 'topic': para['topic'], 'sid': para['sid']})
            for nsid in para['nsid_list']:
                MixedNoteParaList[-1]['en_content'] += Notes[nid][nsid]['en_content'] + '\n'
                MixedNoteParaList[-1]['ch_content'] += Notes[nid][nsid]['ch_content'] + '\n'
        else:
            tmpsidList = [np['sid'] for np in MixedNoteParaList]
            npid = tmpsidList.index(para['sid'])
            for nsid in para['nsid_list']:
                MixedNoteParaList[npid]['en_content'] += Notes[nid][nsid]['en_content'] + '\n'
                MixedNoteParaList[npid]['ch_content'] += Notes[nid][nsid]['ch_content'] + '\n'
                MixedNoteParaList[npid]['topic'] = list(set(MixedNoteParaList[npid]['topic'] + para['topic']))
with open (NOTE_FOLDER + MIXED_FILE_NAME, 'w', newline = '', encoding = 'utf-8') as outFile:
    writer = csv.DictWriter(outFile, fieldnames = ['ch_content', 'en_content', 'topic', 'sid'])
    writer.writeheader()
