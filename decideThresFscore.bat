...
rem 	for %%c in (%chapters1%) do python Evaluate-1.py %1 %subject1% %%c %a1% %b1% %threshold_cossim1% %%t
rem )

for %%t in (%threshold_score_list%) do (
	for %%c in (%chapters2%) do python Evaluate-1.py %1 %subject2% %%c %a2% %b2% %threshold_cossim2% %%t
)